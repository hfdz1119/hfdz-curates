...
export function HomePage({ favoriteIds, toggleFavorite, setFavoriteIds }: { favoriteIds: string[]; toggleFavorite: (id: string) => void; setFavoriteIds: (ids: string[]) => void }) {
  const [params, setParams] = useSearchParams(); const fileRef = useRef<HTMLInputElement>(null);
  ...
  const favorites = resources.filter((resource) => favoriteIds.includes(resource.id)); const filtered = Boolean(filters.query || filters.categoryId || filters.tagId || filters.rating || filters.platform || filters.pricing || filters.license || filters.difficulty || filters.networkAccess);
  return <main>
    <section className="hero"><p className="eyebrow">会飞的猪精选</p><h1>HFDZ Curates <span>| 会飞的猪</span></h1><p>经过筛选，有使用判断，并关联实践笔记。这里保留的是值得回访的工具，而不是一份冗长的链接清单。</p></section>
    <SearchPanel {...filters} categories={categories} tags={tags} onChange={update} />
    <ServiceStrip />
    {!filtered && <><Section title="Featured" caption="有明确编辑判断的长期收录" items={getFeatured()} favoriteIds={favoriteIds} toggle={toggleFavorite} variant="featured" /><Section title="Latest" caption="按录入日期排序" items={getLatest()} favoriteIds={favoriteIds} toggle={toggleFavorite} variant="compact" /></>}
    {favorites.length > 0 && <section className="content-section favorites-section"><div className="section-heading"><div><p className="eyebrow">MY PICKS</p><h2><Heart size={20} fill="currentColor" /> 我的收藏</h2></div><div className="utility-actions"><button onClick={exportFavorites} title="导出收藏"><Download size={15} /> 导出</button><button onClick={() => fileRef.current?.click()} title="导入收藏"><Upload size={15} /> 导入</button><input ref={fileRef} type="file" accept="application/json" onChange={(event) => importFavorites(event.target.files?.[0])} /></div></div><div className="resource-grid">{favorites.slice(0, 6).map((resource) => <ResourceCard key={resource.id} resource={resource} favorite onToggleFavorite={toggleFavorite} />)}</div></section>}
    <section className="content-section catalog-section"><div className="section-heading"><div><p className="eyebrow">CATALOG</p><h2>{filtered ? `找到 ${visible.length} 条资源` : "全部资源"}</h2></div><span>按判断、用途与条件筛选</span></div><div className="resource-grid">{visible.map((resource) => <ResourceCard key={resource.id} resource={resource} favorite={favoriteIds.includes(resource.id)} onToggleFavorite={toggleFavorite} />)}</div>{visible.length === 0 && <div className="empty-state">没有匹配的资源。尝试清除筛选或换一个关键词。</div>}</section>
  </main>;
}
function Section({ title, caption, items, favoriteIds, toggle, variant }: { title: string; caption: string; items: Resource[]; favoriteIds: string[]; toggle: (id: string) => void; variant: "featured" | "compact" }) { return <section className={`content-section ${variant}-section`}><div className="section-heading"><div><p className="eyebrow">CURATED</p><h2>{title}</h2></div><span>{caption}</span></div><div className="resource-grid">{items.map((resource) => <ResourceCard key={resource.id} resource={resource} variant={variant} favorite={favoriteIds.includes(resource.id)} onToggleFavorite={toggle} />)}</div></section>; }
