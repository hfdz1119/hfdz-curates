import { ArrowDown, ArrowLeft, ArrowUp, ChevronDown, ChevronRight, Download, Folder, GripVertical, LibraryBig, LogOut, Pencil, Plus, RotateCcw, Save, ScanSearch, Trash2, Upload, X } from "lucide-react";
...
export function ManagePage({ onBrandIconChange }: { onBrandIconChange: (url: string) => void }) {
  ...
  function addCategory() { const name = categoryName.trim(); if (!name) return; const next = [...categories, { id: `category-${crypto.randomUUID()}`, name, order: categories.length, hidden: false, palette: "aurora" as const, visibility: "public" as const }]; setCategoryName(""); void savePortalConfig(next); }
  ...
  return <main className="portal-page portal-manage-page" id="main-content" data-portal-palette={palette} data-root-only={rootOnly || undefined}>
    ...
    <section className="portal-glass manage-glass" aria-labelledby="manage-title">
      {authenticated === null ? <p className="manage-loading">正在确认管理会话…</p> : !authenticated ? <form className="manage-login" onSubmit={login}>
        ...
        <div className="manage-config-grid">
          ...
          <section className="manage-editor manage-config-section"><div className="manage-editor-heading"><div><h2>文件夹</h2><p>仅支持单层文件夹；删除前必须清空</p></div></div><div className="manage-inline-add"><input aria-label="新文件夹名称" placeholder="新文件夹名称" value={folderName} onChange={(event) => setFolderName(event.target.value)} /><button type="button" onClick={addFolder} disabled={busy}><Plus size={16} />添加</button></div>{folders.slice().sort((a, b) => a.order - b.order).map((folder, index, ordered) => <div className="manage-config-row manage-folder-row" key={folder.id}><input aria-label={`${folder.name}文件夹名称`} value={folder.name} onChange={(event) => setFolders((current) => current.map((item) => item.id === folder.id ? { ...item, name: event.target.value } : item))} /><select aria-label={`${folder.name}所属分类`} value={folder.categoryId} onChange={(event) => setFolders((current) => current.map((item) => item.id === folder.id ? { ...item, categoryId: event.target.value } : item))}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><button type="button" aria-label={`上移 ${folder.name}`} disabled={index === 0} onClick={() => { const next = [...ordered]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; setFolders(next.map((item, order) => ({ ...item, order }))); }}><ArrowUp size={15} /></button><button type="button" aria-label={`下移 ${folder.name}`} disabled={index === ordered.length - 1} onClick={() => { const next = [...ordered]; [next[index], next[index + 1]] = [next[index + 1], next[index]]; setFolders(next.map((item, order) => ({ ...item, order }))); }}><ArrowDown size={15} /></button><button type="button" className="manage-delete" aria-label={`删除文件夹 ${folder.name}`} disabled={busy} onClick={() => void removeFolder(folder)}><Trash2 size={15} /></button></div>)}<button className="manage-primary" type="button" disabled={busy} onClick={() => void savePortalConfig()}><Save size={16} />保存文件夹</button></section>
